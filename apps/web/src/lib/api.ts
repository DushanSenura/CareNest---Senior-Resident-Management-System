...
export type ResidentDetail = Resident & {
  ...
  admissionProfile?: Record<string, unknown> & {
    admissionId?: string;
...
export const api = {
  ...
  dailyHealth: (date?: string) =>
    get<DailyHealthRecord[]>(`/daily-health${date ? `?date=${date}` : ""}`),
  tasks: () => get<CareTaskRecord[]>("/tasks"),
  shifts: (date?: string) =>
    get<ShiftRecord[]>(`/shifts${date ? `?date=${date}` : ""}`),
